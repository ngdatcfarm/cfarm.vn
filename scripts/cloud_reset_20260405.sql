...
CREATE TABLE `barns` (
  ...
  `number` INT COMMENT 'So thu tu chuong 1-9',
  `name` VARCHAR(200) NOT NULL,
  `barn_type` VARCHAR(50) COMMENT 'loại chuồng',
  `area_sqm` DECIMAL(10,2) COMMENT 'Dien tich m2',
  `capacity` INT COMMENT 'Suc chua toi da (con)',
  `capacity_kg` DECIMAL(10,2) COMMENT 'Suc chua toi da (kg)',
...
-- Central warehouse
INSERT INTO `warehouses` (`code`, `name`, `is_central`, `warehouse_type`, `status`)
VALUES ('WH-CENTRAL', 'Kho tong - Thuoc & Vat tu tieu hao', TRUE, 'mixed', 'active');
...
CREATE TABLE `cycles` (
  ...
  `name` VARCHAR(200) NOT NULL,
  `breed` VARCHAR(100),
  `gender` VARCHAR(20) COMMENT 'male, female, mixed',
  `male_quantity` INT DEFAULT 0,
  `female_quantity` INT DEFAULT 0,
  `initial_count` INT NOT NULL COMMENT 'So luong luc bat dau',
  `current_count` INT,
  `purchase_price` DECIMAL(12,2),
