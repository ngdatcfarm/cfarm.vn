...
async function deleteItem(id){if(!confirm('Xóa vật tư này?\n(Chỉ xóa được khi tồn kho = 0)'))return;const ok=await api('/inventory/items/'+id+'/delete',new FormData(),null);if(ok){alert('✅ Đã xóa');location.reload();}else{alert('❌ Xóa thất bại');}}
